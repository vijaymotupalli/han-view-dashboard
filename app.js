(() => {
  ...
  const {
    ...
    formatGenerated, robinhoodLink, CLASS_ORDER, DATA_URL, MARKET_URL,
  } = window.HanDash;
  ...
  function renderHero(best) {
    ...
    $("#hero").innerHTML =
      ...
      '<h2 id="hero-title" class="hero-ticker">' + robinhoodLink(best.ticker, best.ticker, "rh-link hero-rh") + '</h2></div>' +
      '<span class="action-badge ' + actionClass + '">' + escapeHtml(formatAction(best.action)) + '</span></div>' +
      '<div class="levels" role="list">' +
      '<div class="level-card" role="listitem"><span class="label">Current price</span><span class="value">' +
        robinhoodLink(best.ticker, formatPrice(best.current_price), "rh-link mono") + '</span></div>' +
  ...
  function renderDashboard(rows) {
    ...
    body.innerHTML = sorted.map((row) => {
      ...
      return '<tr><td class="ticker-cell">' + robinhoodLink(row.ticker, row.ticker, "rh-link ticker-rh") +
        ...
        '<td class="dir-' + dirClass + '">' + (dir || "-") + '</td>' +
        '<td class="mono">' + escapeHtml(row.entry) + '</td>' +
        '<td class="mono current-price-cell">' + robinhoodLink(row.ticker, priceLabel, "rh-link price-rh") + '</td>' +
  ...
  function renderTickerCards(tickers) {
    ...
    root.innerHTML = tickers.map((t, idx) => {
      ...
      return '<details class="ticker-card"' + openAttr + '><summary><div class="card-summary-left">' +
        '<span class="card-ticker">' + robinhoodLink(t.ticker, t.ticker, "rh-link") + '</span>' +
        ...
        '<p class="rh-card-link">' + robinhoodLink(t.ticker, "Open " + String(t.ticker || "") + " on Robinhood", "rh-link") + '</p>' +
        '</div></div></div></details>';
    }).join("");
  }
